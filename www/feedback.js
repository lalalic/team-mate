(() => {
    ...
    fileInput.addEventListener('change', () => {
        ...
        if (!file.type.startsWith('image/') || file.size > MAX_SCREENSHOT_BYTES) { fileInput.value = ''; preview.removeAttribute('src'); preview.style.display = 'none'; return showError('Choose a PNG, JPEG, or WebP image up to 5 MB.'); }
    ...
    form.addEventListener('submit', async (event) => {
        ...
        if (file && (file.size > MAX_SCREENSHOT_BYTES || !file.type.startsWith('image/'))) return showError('Choose a valid image up to 5 MB.');
